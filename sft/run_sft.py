...
class DataTrainingArguments:
    ...
    output_column: Optional[str] = field(
        default="answer",
        metadata={"help": "The name of the column in the datasets containing the output texts."},
    )
...
def main():
    ...
    if data_args.train_file is not None:
        raw_datasets["train"] = Dataset.from_generator(dataset_gen, gen_kwargs={"filepath": data_args.train_file})  
    if data_args.validation_file is not None:
        raw_datasets["validation"] = Dataset.from_generator(dataset_gen, gen_kwargs={"filepath": data_args.validation_file})
    if data_args.test_file is not None:
        raw_datasets["test"] = Dataset.from_generator(dataset_gen, gen_kwargs={"filepath": data_args.test_file})
    ...
    def preprocess_function(examples):
        ...
        for i in range(len(examples[input_column])):
            if examples[input_column][i] and examples[output_column][i]:
                inputs.append(examples[input_column][i])
                targets.append(examples[output_column][i][0])
    ...
    if training_args.do_predict:
        ...
        if model_args.do_sample:
            # This is used to get examples for feedback collection (we use same decoding parameters during RL exploration)
            predict_results = trainer.predict(
                predict_dataset, metric_key_prefix="predict", max_length=max_length, do_sample=True, temperature=0.7, top_k=20
            )
        else:
            predict_results = trainer.predict(
